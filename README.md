...
When a colour calibration exists, calibrated background removal starts automatically inside the AOI. The mask is cleaned with morphological filtering and the main contour is isolated. Coin and Ring modes also require a reasonably circular contour and display a fitted enclosing circle. Press `B` to toggle between the isolated-object and raw camera views.
