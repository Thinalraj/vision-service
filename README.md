...
Bangle mode keeps the stricter two-boundary detector: it fits the outer contour, then finds and fits the largest elliptical hole inside it. Ring mode is separate and assumes the ring is on a green background. It converts the AOI to HSV, segments the green surface, inverts that mask to find non-green objects, then detects the outer ring boundary first with a fitted ellipse. Inner-hole detection is optional: when contour hierarchy finds a valid inner hole, Ring mode adds inner diameter and annular surface area; when it does not, the outer diameter is still accepted and displayed. It rejects unlikely outer candidates by area, maximum expected physical size, circularity, and ellipse aspect ratio, then scores the best result. Ring mode keeps the raw camera view visible after detection so dark rings do not disappear into a black isolated-mask display; press `B` or the Toggle View button to switch into the isolated mask debug view. The side panel shows outer and inner major/minor axes, equivalent diameters, annular surface area when available, depth, confidence, and inner-hole status.
