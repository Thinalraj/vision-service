...
Press `C` to start colour calibration. Click nine clean background locations spread across the image. Each click samples an 11 x 11 pixel patch; after the ninth point, a robust CIE Lab background profile is saved to `settings.json` for the selected object mode. Press `C` again before completing nine points to cancel.
...
Chain mode uses the calibrated-background mask to estimate projected surface area, rotated bounding-box length and width, axis-aligned footprint width and height, and an approximate chain path length from a skeletonized mask. The path length is an image-based estimate, so it is most useful when the chain is laid flat and not overlapping itself.

Mode settings are stored in `settings.json` under `modes`. Each mode can have its own `aoi` and `color_calibration`, so selecting a mode automatically loads and crops to that mode's saved AOI. Older `vision_config.json` files are migrated automatically the first time the app starts.

Press `S` after detection or a manual two-point measurement to save a result. The first save asks for the CSV destination and stores that path in `settings.json`. Every subsequent save asks only for the item name and appends one new row to the same CSV. Press `O` to select a different results file. Each row contains the timestamp, item name, object type, diameter, projected surface area, estimated camera depth, and AOI coordinates. A manual two-point diameter takes priority over the fitted-circle estimate. Bar results also include rotated-rectangle width, length, top surface area, and estimated thickness. Thickness is calculated from the median background depth minus the median object depth inside the AOI.
